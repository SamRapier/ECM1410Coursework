...
/**
 * An interface for all functionalities of a hotel management system.
 *
 * @author Hongping cai
 * @version 15/02/2019
 *
...
public interface Hotel {
  ...
  /**
   * Make a booking for one room type. If more than one room avaible, choose one
   * room randomly to book
   *
   * @param guestID  a unique guest ID
   * @param roomType a room type
   * @param checkin  the check-in date
   * @param checkout the check-out date
   * @return the booked room number if the booking is successful, otherwise,
   *         return -1
   */
  ...
  /**
   * Print out a guest's booking information (if any) by given the unique guest
   * ID.
   *
   * @param guestID a unique guest ID
   *
  ...
  /**
   * Display on the screen all the booking information by given a date The output
   * contains booking ID, the guest name, the room number, the room type, the room
   * price, the payment price (if the guest is a VIP, the payment price is
   * different from the room price, otherwise, it equals the room price)
   *
  ...
  /**
   * Display on the screen all the payments on a given date. This is to check how
   * much income on a specific day. The output contains the guest ID, the payment
   * amount, the payment reason ("booking", "VIPmembership" or "ref nd"). If it is
   * due to cancelling, the payment amount refers to the refund amount – a n
   * gative value.
   *
   * @param thisDate a given date
   */
